...
const HealthTracking = () => {
    ...
    const getPredictionForLatest = async () => {
        ...
        const predictiveInput = {
            heart_rate: latest.heartRate?.value ?? 0,
            bp_systolic: latest.bloodPressure?.systolic ?? 0,
            bp_diastolic: latest.bloodPressure?.diastolic ?? 0,
            glucose: latest.bloodSugar?.value ?? 0,
            sleep_hours: latest.sleepHours ?? 7,
            temperature: latest.temperature ?? 98.6,
            oxygen_level: latest.oxygenLevel ?? 98,
            age: latest.age ?? 65,
        };
        try {
            const res = await predictiveApi.predictRisk(predictiveInput);
                    ...
                    <>
                        ...
                        {prediction && !prediction.error && (
                            ...
                            >
                                ...
                                <p className="text-base mb-1 text-gray-800">{prediction.explanation}</p>

                                {prediction.risk_factors?.length > 0 && (
                                    <div className="mt-2">
                                        <span className="font-medium text-gray-700">Key Risk Factors: </span>
                                        <ul className="list-disc list-inside text-sm text-gray-700">
                                            {prediction.risk_factors.slice(0, 4).map((factor, idx) => (
                                ...
                                <div className="mt-4">
                                    ...
                                    <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                        {(prediction.risk_factors.includes('Insufficient sleep')) && (
                                            <li>Try to get 7-8 hours of quality sleep each night.</li>
                                        )}
                                        {(prediction.risk_factors.includes('High systolic blood pressure') || prediction.risk_factors.includes('High diastolic blood pressure')) && (
                                            <li>Reduce salt intake and monitor blood pressure regularly.</li>
                                        )}
                                        {(prediction.risk_factors.includes('High heart rate (tachycardia)') || prediction.risk_factors.includes('Low heart rate (bradycardia)')) && (
                                            <li>Avoid stimulants like caffeine and seek medical advice if irregular.</li>
                                        )}
                                        {(prediction.risk_factors.includes('Elevated blood glucose') || prediction.risk_factors.includes('Low blood glucose')) && (
                                            <li>Maintain balanced nutrition and monitor sugar levels.</li>
                                        )}
                                        {prediction.risk_factors.includes('Low oxygen saturation') && (
